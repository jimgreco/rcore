...
impl Host {
    ...
    pub fn cache_instance(&mut self, instance: class::Instance) -> u64 {
        ...
        self.instances.insert(self.id, instance);
        self.id
    }

    pub fn make_instance(&mut self, name: &str, fields: Vec<PolarValue>) -> crate::Result<u64> {
        let class = self.get_class(name)?.clone();
        let fields = fields;
        let instance = class.init(fields)?;
        Ok(self.cache_instance(instance))
    }
